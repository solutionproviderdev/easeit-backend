		...
		statusHistory: [
			{
				...
				changedBy: {
					type: mongoose.Schema.Types.ObjectId,
					ref: 'User',
					required: false, // you can make this required if you always have a user context
				},
			},
		],
...
leadSchema.index({ 'statusHistory.status': 1 });
...
leadSchema.pre('save', function (next) {
	if (this.isModified('status')) {
		this.statusHistory.push({
			status: this.status,
			changedAt: new Date(),
			changedBy: this.$locals.userId || null,
		});
		console.log('Appended to statusHistory:', this.statusHistory.slice(-1)[0]);
	}
	next();
});

// Update statusHistory on findOneAndUpdate
leadSchema.post('findOneAndUpdate', async function (doc) {
	if (!doc) return;

	const updatedStatus = this.getUpdate().status || this.getUpdate().$set?.status;
	if (!updatedStatus) return;

	const userId = this.getOptions().context?.userId || null;

	// Push into statusHistory manually
