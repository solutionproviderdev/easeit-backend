...
const processPurchaseItems = async (items, vendorId) => {
    ...
    return Promise.all(
        items.map(async (item) => {
            ...
            if (!vm) throw new Error(`Vendor material not found: ${item.vendorMaterial}`);
...
exports.getAllPurchases = async (req, res, next) => {
    try {
        const {
 page = 1, limit = 10, status, vendor, startDate, endDate 
} = req.query;
        const query = {};
...
exports.addPayment = async (req, res, next) => {
    try {
        ...
        const paidSoFar = p.paymentDetails.reduce((sum, pay) => sum + pay.amount, 0);
...
exports.updatePayment = async (req, res, next) => {
    try {
        ...
        const totalPaid = p.paymentDetails.reduce((sum, pay) => sum + pay.amount, 0);
