...
exports.addPayment = async (req, res) => {
 ...
 amount, paymentMethod, paymentDate, paymentStatus, paymentNote 
        ...
        // Retrieve the newly added payment (last element)
        const addedPayment = lead.finance.payments[lead.finance.payments.length - 1];
        res.status(201).json({
...
exports.updatePayment = async (req, res) => {
 ...
 amount, paymentMethod, paymentDate, paymentStatus, paymentNote 
        ...
        if (paymentNote !== undefined) lead.finance.payments[paymentIndex].paymentNote = paymentNote;
...
exports.deletePayment = async (req, res) => {
    try {
        ...
        const originalCount = lead.finance.payments.length;
        lead.finance.payments = lead.finance.payments.filter(
            (payment) => payment._id.toString() !== paymentID
        );
        if (lead.finance.payments.length === originalCount) {
            return res.status(404).json({ error: 'Payment not found' });
        }
