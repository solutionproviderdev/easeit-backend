...
exports.getAllLeads = async (req, res) => {
	try {
		...
		leads.forEach(lead => {
			...
			if (
				lead.salesExqName &&
				!salesExecsSet.has(lead.salesExqName._id.toString())
			) {
...
exports.createLead = async (req, res) => {
	const { name, phone, source, status, comment, images, cre, productAd } =
		req.body;
...
exports.updateRequirements = async (req, res) => {
	...
	try {
		...
		console.error(
			`Error updating requirements for lead ${id}: ${error.message}`
		);
		res.status(500).json({ msg: 'Server error' });
	}
};
...
exports.addPhoneNumberToLead = async (req, res) => {
	...
	try {
		...
		if (!parsedNumber || !parsedNumber.isValid()) {
			return res
				.status(400)
				.json({ msg: 'Invalid phone format put bd number' });
		}
		...
		if (lead.phone.includes(formattedPhoneNumber)) {
			return res
				.status(400)
				.json({ msg: 'Phone number already exists for this lead' });
		}
...
exports.updateLead = async (req, res) => {
	...
	if (req.body.projectStatus)
		updateFields.projectStatus = req.body.projectStatus;
	if (req.body.projectLocation)
		updateFields.projectLocation = req.body.projectLocation;
	if (req.body.messagesSeen !== undefined)
		updateFields.messagesSeen = req.body.messagesSeen;
	...
	try {
		...
		res
			.status(200)
			.json({ msg: 'Lead updated successfully', lead: updatedLead });
	} catch (error) {
...
exports.addReminder = async (req, res) => {
	...
	try {
		...
		res
			.status(200)
			.json({ msg: 'Reminder added successfully', reminders: lead.reminder });
	} catch (error) {
...
exports.addReminderWithComment = async (req, res) => {
	...
	try {
		...
		console.error(
			`Error adding reminder with comment to lead ${id}: ${error.message}`
		);
		res.status(500).json({ msg: 'Server error' });
	}
};
...
exports.updateReminderStatus = async (req, res) => {
	...
	try {
		...
		res
			.status(200)
			.json({ msg: 'Reminder status updated successfully', reminder });
	} catch (error) {
		console.error(
			`Error updating reminder status for lead ${leadId}: ${error.message}`
		);
		res.status(500).json({ msg: 'Server error' });
	}
};
...
exports.addCallLog = async (req, res) => {
	const { id } = req.params;
	const { recipientNumber, callType, status, callDuration, timestamp } =
		req.body;
...
exports.getAllLeadsWithReminders = async (req, res) => {
	try {
		const { status, source, startDate, endDate, assignedCre, salesExecutive } =
			req.query;
		...
		const isCRE = user.departmentId?.roles?.some(
		...
		if (startDate && endDate) {
			if (start > end) {
				return res
					.status(400)
					.json({ msg: 'startDate cannot be after endDate.' });
			}
			if (startDate === endDate) {
		...
		const populatedLeads = leads.map(lead => {
			const filteredReminders = lead.reminder.filter(reminder => {
				...
				return (
					(!start || reminderTime >= start) && (!end || reminderTime <= end)
				);
			});
		...
		const filteredLeads = populatedLeads.filter(
