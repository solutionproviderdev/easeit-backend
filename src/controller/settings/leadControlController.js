		...
		const creDistribution = await Promise.all(
			creUsers.map(async user => {
				...
				return {
					...
					manualLeadAssignEndTime: override
						? override.manualLeadAssignEndTime
						: null,
				};
			})
		);
			...
			if (cre.manual && cre.manualLeadAssignRate) {
				...
				const adjustedRate =
					totalPerformanceNonManual > 0
						? (cre.performance / totalPerformanceNonManual) *
						  (100 - totalManualRate)
						: 0;
...
exports.createManualOverride = async (req, res) => {
	try {
		...
		if (!settings) {
			return res
				.status(400)
				.json({ error: 'Manual override already exists for this CRE' });
		}
...
exports.updateAutoMessage = async (req, res) => {
	try {
		...
		console.log('Updating auto message with:-----:>', autoMessage);
		...
		if (
			...
			return res
				.status(400)
				.json({ error: 'Invalid autoMessage data provided' });
		}
