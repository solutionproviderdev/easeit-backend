...
const createGlass = async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ message: 'Failed to create glass', error: err.message });
	}
};
...
const updateGlass = async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ message: 'Failed to update glass', error: err.message });
	}
};
...
const deleteGlass = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: 'Failed to delete glass', error: err.message });
	}
};

const getAllGlass = async (req, res) => {
	const { search, type, color, thickness, sort, limit, page, fields } =
		req.query;
	...
	try {
		...
		res
			.status(500)
			.json({ message: 'Failed to fetch glasses', error: err.message });
	}
};
...
const getGlassById = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: 'Failed to fetch glass', error: err.message });
	}
};
