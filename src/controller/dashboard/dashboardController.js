...
const getAllCREsPerformanceData = async (req, res) => {
	try {
		...
		if (!creRole) {
			return res
				.status(404)
				.json({ message: 'CRE role not found in CRE department' });
		}
		...
		crePerformanceData.sort(
			...
			(a, b) =>
				b.performanceMetrics.completePerformance -
				a.performanceMetrics.completePerformance
		);
...
const getMeetingsData = async (req, res) => {
	try {
		...
		if (timeLength === 'week') {
			...
			daysArray = Array.from({ length: endDate.date() }, (_, i) =>
				(i + 1).toString()
			);
		} else {
		...
		const meetingsData = Object.entries(groupedMeetings).map(
			([day, value]) => ({
				day,
				value,
			})
...
const getDateWiseLeadData = async (req, res) => {
	try {
		...
		while (currentLocal.isSameOrBefore(endLocal)) {
			...
			daysArray.push(
				currentLocal.clone().utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')
			);
		...
		meetings.forEach(meeting => {
			...
			if (groupedData[meetingDateKey]) {
				if (meeting.isMeetingCompleted)
					groupedData[meetingDateKey].meetingsCompleted += 1;
				if (meeting.isMeetingSold)
					groupedData[meetingDateKey].meetingsSold += 1;
			}
		});
...
const getLeadOverview = async (req, res) => {
	...
	try {
		...
		// 3) your Meeting‐based stats (filter by lead ∈ assignedLeadIds and meeting.date)
		const leadsMeetingSet = await Meeting.countDocuments({
			lead: { $in: assignedLeadIds },
			date: { $gte: start, $lte: end },
			status: 'Fixed',
		});
		const leadsMeetingComplete = await Meeting.countDocuments({
			lead: { $in: assignedLeadIds },
			date: { $gte: start, $lte: end },
			status: 'Complete',
		});
		// const leadsMeetingPostponed = await Meeting.countDocuments({
		// 	lead: { $in: assignedLeadIds },
		// 	date: { $gte: start, $lte: end },
		// 	status: 'Postponed',
		// });
		// const leadsMeetingRescheduled = await Meeting.countDocuments({
		// 	lead: { $in: assignedLeadIds },
		// 	date: { $gte: start, $lte: end },
		// 	status: 'Rescheduled',
		// });
		// const leadsMeetingCanceled = await Meeting.countDocuments({
		// 	lead: { $in: assignedLeadIds },
		// 	date: { $gte: start, $lte: end },
		// 	status: 'Canceled',
		// });
		...
		const finalResponse = [
			{ name: 'Total Assigned', value: totalLeads },
			{ name: 'Number Collected', value: leadsWithPhone },
			...
			{ name: 'Sold', value: leadsSold },
			{ name: 'Close', value: leadsClose },
		];
...
const getFollowUpStats = async (req, res) => {
	...
	try {
		...
		const creFollowUpPerformance = await getCREFollowUpPerformance(
			creID,
			start,
			end
		);
